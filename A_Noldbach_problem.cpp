...
vector<ll> sieveOfEratosthenes(ll n) { vector<bool> is_prime(n + 1, true); vector<ll> primes;is_prime[0] = is_prime[1] = false;for (ll p = 2; p * p <= n; p++) {if (is_prime[p]) {
for (ll i = p * p; i <= n; i += p) {is_prime[i] = false; }}}for (ll p = 2; p <= n; p++) {if (is_prime[p]) {primes.push_back(p);}}
return primes;
}
