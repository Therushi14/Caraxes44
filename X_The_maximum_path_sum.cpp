...
int fun(int i,int j , int n ,int m , vector<vector<int>> &v){
    if(i==n-1 and j== m-1){
        return v[i][j];
    }
    ...
    if(i<n-1){
        if(j<m-1){
            return max(fun(i+1,j,n,m,v)+v[i][j],fun(i,j+1,n,m,v)+v[i][j]);
        } else {
            return fun(i+1,j,n,m,v)+v[i][j];
        }
    } else{
        return fun(i,j+1,n,m,v)+v[i][j];
    }
}
int32_t main()
{
