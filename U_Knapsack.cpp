...
int fun(int idx,int left , vector<int> weight,vector<int> value,int n){
    if(idx==n) {return 0;}
    if(left>=weight[idx]){
        return max(fun(idx+1,left,weight,value,n),fun(idx+1,left-weight[idx],weight,value,n)+value[idx]);
    } else {
        return fun(idx+1,left,weight,value,n);
    }
}
int32_t main()
{
